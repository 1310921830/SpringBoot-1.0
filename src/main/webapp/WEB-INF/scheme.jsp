    ...
    <FORM id="form">
			...
			<TBODY>
				<TR ALIGN=center VALIGN=middle ID=TableTitle>
					...
					<TD STYLE="border-bottom: 1px solid #94C6E7;">
						<c:out value="${scheme.title}"></c:out>
					</TD>
		...
		<TABLE CELLSPACING="0" CELLPADDING="0" CLASS="TableStyle" STYLE="width: 90%">
			...
			<td colspan="6" align="left" STYLE="border-bottom: 1px solid #94C6E7;">
				<textarea name="content" rows="6" readonly="readonly"><c:out value="${scheme.content}"></c:out>
				</textarea> 
				</td>
			</tr>
		...
		<c:if test="${scheme.preparedName eq user.userId}">
				...
				STYLE="width: 90%">
					...
					<tr ALIGN=center VALIGN=middle ID=TableTitle>
						...
						<textarea name="opinion" rows="6" class="i1" readonly="readonly"><c:out value="${scheme.opinion}"></c:out>
						</textarea> 
						</td>
					</tr>
					</TABLE>
		</c:if>
		<c:if test="${scheme.reviewedName eq user.userId}">
				<c:if test="${'是' eq scheme.isCheck }">
						<TABLE CELLSPACING="0" CELLPADDING="0" CLASS="TableStyle"
				STYLE="width: 90%">
						<TR ALIGN=center VALIGN=middle ID=TableTitle>
